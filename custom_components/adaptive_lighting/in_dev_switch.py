...
_ORDER = (SUN_EVENT_DAWN, SUN_EVENT_SUNRISE, SUN_EVENT_NOON, SUN_EVENT_SUNSET, SUN_EVENT_DUSK, SUN_EVENT_MIDNIGHT)
...
class AdaptiveSwitch(SwitchEntity, RestoreEntity):
    ...
    async def _adapt_light(
        ...
        if lock is not None and lock.locked():
            _LOGGER.warning("%s: '%s' is locked", self._name, light)
            return
...
class SunLightSettings:
    ...
    def get_sun_events(self, date: datetime.datetime) -> Dict[str, float]:
        ...
        if self.sunrise_time is None and self.sunset_time is None:
            ...
            dawn = self.sunrise_time - 3600  # one hour static dawn time
            dusk = self.sunset_time + 3600  # one hour static dusk time

            _LOGGER.warning("Could not get infos from astral function, so infos has to be calculated manually.")
        ...
        #_LOGGER.info("events: '%s'", events)
        #_LOGGER.info("SUN_EVENT_SUNRISE 1: '%s'", SUN_EVENT_SUNSET[0,0])
        return events
    ...
    def calc_percent(self, transition: int) -> float:
        ...
        _LOGGER.info("_: '%s'", _)
        _LOGGER.info("next_event: '%s', next_ts: '%s'", next_event, next_ts)
...
#########################################
    ...
    def calc_brightness_pct(self, percent: float, is_sleep: bool) -> float:
        ...
        # Updated behavior for a more natural brightness transistion using extra time period before and after sunrise and Sunset.
        # Sunrise: Start of increasing Brightness is set to Dawn.
        # Due to the natural behavior, that with sunrise, the sun brightness is not at maximum, a addition dawn period (6 degrees) is used
        # to extend the brightness transistion to max brightness.
        now = dt_util.utcnow()

        # dawnDelta = SUN_EVENT_SUNRISE - SUN_EVENT_DAWN
        dawnDelta = 3600

        # duskDelta = SUN_EVENT_DUSK - SUN_EVENT_SUNSET
        duskDelta = 3600

        # Calc Daylight to max_brightness
        if now > SUN_EVENT_SUNRISE + dawnDelta and now < SUN_EVENT_SUNSET + duskDelta:
            return self.max_brightness

        # Calc Night to min_brightness
        if now < SUN_EVENT_DAWN or now > SUN_EVENT_DUSK:
