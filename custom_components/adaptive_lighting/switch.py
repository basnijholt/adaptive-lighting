...
class AdaptiveSwitch(SwitchEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._last_lux_brightness: float | None = None
    ...
    def _set_changeable_settings(
        ...
        self._last_lux_brightness = None
    ...
    def _get_smoothed_lux(self) -> float | None:
        ...
        valid_samples = [
            value for timestamp, value in self._lux_samples if timestamp >= cutoff
        ]

        if not valid_samples:
            _, most_recent_value = self._lux_samples[-1]
    ...
    async def _lux_sensor_state_event_action(
        ...
        if self._sun_light_settings.brightness_mode != "lux" or not self.is_on:
            return

        smoothed_lux = self._get_smoothed_lux()
        if smoothed_lux is not None:
            new_brightness = self._sun_light_settings._brightness_pct_lux(
                smoothed_lux,
            )
            if (
                self._last_lux_brightness is not None
                and abs(new_brightness - self._last_lux_brightness) < 1.0
            ):
                return
            self._last_lux_brightness = new_brightness
